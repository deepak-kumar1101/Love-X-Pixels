      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-indigo-500/30 bg-background/95 p-6 shadow-2xl backdrop-blur-xl dark:border-indigo-500/40 dark:bg-zinc-900/95 sm:p-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Authenticate using your Discord account via Supabase Auth to join events, claim rewards,
        ...
        <div className="mt-8 space-y-4">
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center space-x-3 rounded-2xl bg-[#5865F2] py-3.5 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:bg-[#4752C4] hover:shadow-indigo-500/25 active:scale-[0.98] disabled:opacity-50"
          >
            {isLoading ? (
        ...
        {/* Security & Roles Footer Notice */}
        <div className="mt-6 border-t border-border/40 pt-4 text-center">
          <div className="inline-flex items-center space-x-1.5 text-[11px] font-medium text-muted-foreground">
            <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" />
            <span>Official Supabase 1-Click Discord OAuth</span>
          </div>
        </div>
      </div>
    </div>
